...
# Ask the user how much money they would have invested in each stock on the starting date
investment_per_stock = []
for ticker in tickers:
    investment = float(input(f"How much money would you have invested in {ticker} on {start_date} (in INR): "))
    investment_per_stock.append(investment)

# Calculate the price of each stock on the starting date (first date of data)
initial_prices = data.iloc[0]

# Calculate how many shares could have been bought with the initial investment
shares_bought = np.array(investment_per_stock) / initial_prices

# Calculate the final value of the investment (price on the end date * number of shares)
final_prices = data.iloc[-1]
final_values = shares_bought * final_prices
...
print(f"Total invested money: ₹{sum(investment_per_stock):.2f}")
print(f"Total profit/loss: ₹{profit_or_loss:.2f}")
print(f"Total portfolio value (Investment + Profit/Loss): ₹{total_value:.2f}")
...
# Modify the total investment and profit/loss calculations here
