...
investment_amount = float(input("Enter your total investment amount (in INR): "))

# Get user input for the portfolio allocation (ensure the sum of weights is 1)
print("Enter the percentage allocation for each stock (must sum to 100):")
allocations = []
for ticker in tickers:
    allocation = float(input(f"Enter allocation for {ticker}: "))
    allocations.append(allocation)

# Normalize the allocation to ensure the sum is 100
allocation_sum = sum(allocations)
allocations = [allocation / allocation_sum for allocation in allocations]

# Calculate the amount invested in each stock based on user allocation
investment_per_stock = [allocation * investment_amount for allocation in allocations]

# Calculate how much each stock has grown or shrunk
final_prices = data.iloc[-1]
initial_prices = data.iloc[0]

# Calculate final value of each stock in portfolio
...
profit_or_loss = total_value - investment_amount
print(f"Total portfolio value: ₹{total_value:.2f}")
print(f"Total profit/loss: ₹{profit_or_loss:.2f}")

# --- 8. Pie Chart of Custom Portfolio Allocation ---
plt.figure(figsize=(8, 8))
plt.pie(allocations, labels=tickers, autopct='%1.1f%%', startangle=90, colors=sns.color_palette('pastel'))
plt.title(f'Portfolio Allocation (Custom Weights based on User Input)')
